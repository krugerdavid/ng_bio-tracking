...
export class BioimpedanceDomainService {
  ...
  validateBioimpedance(bioimpedance: Partial<Bioimpedance>): { isValid: boolean; errors: string[] } {
    ...
    if (
      bioimpedance.bodyFatPercentage !== undefined &&
      !this.validateBodyFatPercentage(bioimpedance.bodyFatPercentage)
    ) {
      errors.push("Body fat percentage must be between 0 and 100");
    }

    if (
      bioimpedance.muscleMassPercentage !== undefined &&
      !this.validateMuscleMassPercentage(bioimpedance.muscleMassPercentage)
    ) {
    ...
    if (
      bioimpedance.basalMetabolicRate !== undefined &&
      (bioimpedance.basalMetabolicRate <= 0 || bioimpedance.basalMetabolicRate > 10000)
    ) {
