...
interface RegisterMemberPageProps {
    onSubmit: (e: FormEvent, formData: { name: string; email: string; dateOfBirth: string; gender: 'male' | 'female' | 'other' }) => Promise<void>;
    error: string;
    loading: boolean;
}

export function RegisterMemberPage({ onSubmit, error, loading }: RegisterMemberPageProps) {
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="flex gap-4 pt-4">
                        ...
                        <button
                            type="button"
                            onClick={() => navigate('/')}
                            className="px-6 py-3 border-2 border-gray-300 bg-white text-gray-700 font-semibold rounded-lg hover:bg-gray-50 transition-all duration-300"
                        >
                            Cancelar
                        </button>
                    </div>
